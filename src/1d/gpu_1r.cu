...
__constant__ double param_matrix_1d1r_d[16 * 8];

__global__ void kernel_1d1r(const double *__restrict__ in, double *__restrict__ out) {
...
#pragma unroll
    for (int i = 0; i < 4; i++) {
        wmma::load_matrix_sync(param_frag[i], param_matrix_1d1r_d + i * 32, 8);
    }
...
#pragma unroll
    for (int row = 0; row < 2; row++) {
        wmma::load_matrix_sync(in_frag, sharedmem + warp_begin + 4 * row,
                               D_BLOCK_SIZE_COL / 8);
...
#pragma unroll
        for (int row = 2; row < 4; row++) {
            wmma::load_matrix_sync(in_frag, sharedmem + warp_begin + 8 * data + 4 * row,
                                   D_BLOCK_SIZE_COL / 8);
        ...
        wmma::store_matrix_sync(
                out + begin + warp_begin + IDX(data, 4, 8),
                        acc_frag[0], D_BLOCK_SIZE_COL / 8, wmma::mem_row_major);
...
#pragma unroll
    for (int row = 2; row < 4; row++) {
        wmma::load_matrix_sync(in_frag, sharedmem + warp_begin + 8 * data + 4 * row,
                               D_BLOCK_SIZE_COL / 8);
        wmma::mma_sync(acc_frag[1], in_frag, param_frag[row], acc_frag[1]);
    }
    wmma::store_matrix_sync(
            out + begin + warp_begin + IDX(data, 4, 8),
                    acc_frag[1], D_BLOCK_SIZE_COL / 8, wmma::mem_row_major);

}

void
gpu_1d1r(const double *__restrict__ in, double *__restrict__ out, const double *__restrict__ params, const int times,
         const int input_n) {
    ...
    for (int col = 0; col < 8; col++) {
        for (int row = 0; row < 7; row++) {
            param_matrix_h[(row + col + 1) * 8 + col] = params[row];
        }
    }

    CUDA_CHECK(cudaMemcpyToSymbol(param_matrix_1d1r_d, param_matrix_h, 16 * 8 * sizeof(double)));
    ...
    for (; i < times; i++) {
        CUDAKERNELCHECK(
                (kernel_1d1r<<<grid_config, block_config>>>(array_d[i % 2], array_d[(i + 1) % 2])));
    }
    CUDA_CHECK(cudaDeviceSynchronize());

    std::chrono::steady_clock::time_point end = std::chrono::steady_clock::now();
    std::cout << "LoRAStencil(1D 1d1r): " << std::endl;
