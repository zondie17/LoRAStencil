...
#pragma unroll
    for (int row = 0; row < 3; row++) {
        ...
        wmma::load_matrix_sync(in_frag1, data + IDX2D(4 * row, 8 * warp_id + 4, D_BLOCK_SIZE_COL),
                               D_BLOCK_SIZE_COL);
...
#pragma unroll
    for (int row = 0; row < 3; row++) {
        ...
        wmma::load_matrix_sync(in_frag2, data + IDX2D(2, 8 * warp_id + 4 * row, D_BLOCK_SIZE_COL),
                               D_BLOCK_SIZE_COL);
...
void gpu_star_3d1r(const double *__restrict__ in, double *__restrict__ out, const double *__restrict__ params,
    ...
    for (int col = 0; col < 8; col++) {
        for (int row = 0; row < 3; row++) {
            param_matrix_vertical_h[(row + col) * 8 + col] = 1;
        }
    }
